...
# Default test database URL - in-memory SQLite for fast, isolated tests
...
def _create_cleanup_engine(test_db_url):
    ...
    Args:
        test_db_url (str): Database URL string in SQLAlchemy format
                          (e.g., 'sqlite:///:memory:', 'sqlite:///path/to/db.sqlite',
                           'postgresql://user:pass@host:port/dbname')
        
    Returns:
        sqlalchemy.engine.Engine: SQLAlchemy engine instance configured appropriately 
...
def _cleanup_database(test_db_url, phase="session"):
    ...
    Args:
        test_db_url (str): Database URL string in SQLAlchemy format
        phase (str): Description of when cleanup is happening (for logging)
    """
...
def pytest_sessionstart(session):
    ...
    """
    # Get DATABASE_URL from environment or use default in-memory SQLite
    test_db_url = os.getenv('DATABASE_URL', DEFAULT_TEST_DB_URL)
